...
- **OMP loader imports stay literal**: `packages/omp/extensions/archives.ts` must `import("../../../src/tool-operations.ts")` with a `.catch` fallback to `dist`. An `import(url.href)` built from `existsSync` loses `c12` resolution in the compiled OMP 18 binary. Pi may keep the existsSync form.
