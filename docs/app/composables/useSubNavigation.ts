...
const NAV_ICONS: Record<string, string> = {
  ...
  "/guide/snapshots": "i-lucide-history",
  "/guide/content": "i-lucide-file-text",
  "/guide/diff": "i-lucide-diff",
...
function withIcons(items: ContentNavigationItem[]): ContentNavigationItem[] {
  return items.map((item) => ({
    ...
    /** Leaf pages match exactly, so /guide is not highlighted together with /guide/snapshots. */
