...
- Memento is not in `providers.all()`. MemGator already asks the Wayback Machine, Archive.today and others, so adding it to `all` would double the requests to those archives.
