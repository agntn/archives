...
Environment-specific overrides live under `$development`, `$production` and `$test`. `getConfig()` returns the resolved object, `resetConfig()` drops the cache, `resolveConfig({ cwd })` loads from somewhere else without caching.
